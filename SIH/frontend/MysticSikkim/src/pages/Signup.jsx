    ...
    return (
      <div className="relative flex items-center justify-center">
        <div className="absolute inset-0 flex items-center justify-center">
          <div
            className={`w-80 h-80 rounded-full border-2 border-amber-400/30 ${showAura ? "scale-110 opacity-60" : "scale-100 opacity-30"}`}
            style={{ transitionProperty: "transform, opacity", transitionDuration: "6000ms" }}
          >
        ...
        <div
          className={`relative z-10 ${breathe ? "scale-105" : "scale-100"}`}
          style={{ transitionProperty: "transform", transitionDuration: "4000ms" }}
        >
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className={`flex ${isMobile ? "flex-col" : "flex-row"} w-full max-w-6xl rounded-3xl overflow-hidden shadow-2xl backdrop-blur-md bg-white/5 border border-white/10`}>
          {!isMobile && (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[700px] p-12 bg-gradient-to-br from-amber-500/10 via-orange-600/10 to-yellow-500/10 backdrop-blur-md border-r border-white/10 relative">
              <div className="relative z-10 text-center">
                ...
                <div className="mt-8 space-y-4">
                  <h3 className="text-white text-3xl font-bold bg-gradient-to-r from-amber-300 to-yellow-300 bg-clip-text text-transparent">Discover Inner Peace</h3>
                  <p className="text-white/80 text-lg leading-relaxed max-w-md">Begin your spiritual journey through the sacred monasteries of Sikkim. Find wisdom, peace, and enlightenment.</p>
                  <div className="mt-6 h-16 flex items-center justify-center">
                    <p className="text-amber-200/80 text-sm italic font-light text-center px-4 animate-pulse">"The mind is everything. What you think you become." - Buddha</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={`flex-1 flex flex-col justify-center ${isMobile ? "p-8" : "p-16"} bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-md relative`}>
            {isMobile && (
              <div className="text-center mb-8">
                ...
                <h3 className="text-white text-2xl font-bold mb-2 bg-gradient-to-r from-amber-300 to-yellow-300 bg-clip-text text-transparent">Join MysticSikkim</h3>
            ...
            {!isMobile && (
              <div className="text-center mb-10">
                <h2 className="text-4xl font-bold text-white mb-4 bg-gradient-to-r from-amber-300 via-yellow-300 to-amber-300 bg-clip-text text-transparent">Begin Your Journey</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {[
                ...
                <div key={field} className="relative group">
                  <div className="absolute left-4 top-1/2 transform -translate-y-1/2 text-amber-300/70 text-lg z-10">{icon}</div>
                  <input
                    type={type}
                    name={field}
                    ...
                    className={`w-full pl-14 pr-6 py-4 bg-white/10 backdrop-blur-md border-2 rounded-2xl text-white placeholder-white/50 transition-all duration-300 ${focusedField === field ? "border-amber-400 bg-white/20 shadow-lg shadow-amber-500/20 scale-105" : "border-white/20 hover:border-amber-300/50"}`}
                    required
                  />
                  <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r from-amber-400/20 to-yellow-400/20 opacity-0 transition-opacity duration-300 pointer-events-none ${focusedField === field ? "opacity-100" : ""}`}></div>
                </div>
              ))}
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full py-4 rounded-2xl font-bold text-lg text-white shadow-lg transition-all duration-300 relative overflow-hidden group ${loading ? "opacity-70 cursor-not-allowed bg-amber-600/50" : "bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-500 hover:to-yellow-500 hover:scale-105 hover:shadow-xl hover:shadow-amber-500/30 active:scale-95"}`}
              >
              ...
              <div className="text-center pt-6 border-t border-white/10">
                ...
                <NavLink to="/dashboard" className="inline-block text-amber-400 font-semibold hover:text-yellow-300 transition-all duration-300 hover:scale-105 px-4 py-2 rounded-lg hover:bg-white/5">✨ Continue Your Journey</NavLink>
              </div>
            </form>
          </div>
        </div>
